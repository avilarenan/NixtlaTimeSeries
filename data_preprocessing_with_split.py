...
for dataset_name in tqdm(datasets_names, desc="Datasets"):
    ...
    Y_df = LongHorizon2.load(directory='data', group=dataset_name, normalize=False)
    ...
    test_dataset_size = ts_metadata[dataset_name]["test_size"]
    ...
    df_raw_train = df_raw[:-test_dataset_size].copy()
    df_raw_test = df_raw[-test_dataset_size:].copy()

    for dataset_split_part_name, df_raw in [("train", df_raw_train), ("test", df_raw_test)]:
        ...
        df.to_parquet(f"./processed_data/{dataset_name}_{dataset_split_part_name}.parquet", index=False)